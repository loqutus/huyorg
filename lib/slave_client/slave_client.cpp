...
std::unordered_map<std::string, std::string> slave_client::get_containers(){
    ...
    auto containers_map = json_obj.get_map(); 
    return containers_map;
}

std::string slave_client::run_container(std::string image, std::string command){
    std::string request = "";
