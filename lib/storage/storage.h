...
public:
    ~storage();
    int set_slave(std::string host_port, std::unordered_map<std::string, std::string> value); //key-slave host, port; value=slave parameters
    int set_pod(std::string key, std::unordered_map<std::string, std::string> value); //key-pod name, value=pod parameters
    int set_container(std::string key, std::unordered_map<std::string, std::string> value); //key=pod name, value key = container id, value value = container status
